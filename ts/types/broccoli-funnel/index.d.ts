declare module 'broccoli-funnel' {
  ...
  class BroccoliFunnel extends BroccoliPlugin {
    constructor(inputNode: BroccoliNode, options?: BroccoliFunnel.FunnelOptions)
  }

  namespace BroccoliFunnel {
    interface FunnelOptions {
       ...
       * if the `srcDir` were missing an error would be thrown. Setting `allowEmpty` to `true` will
       * prevent that error by creating an empty directory at the destination path.
       */
      allowEmpty?: boolean;

      /**
       * One or more matcher expression (regular expression, glob string, or function). Files within
       * the node whose names match this expression will be copied (with the location inside their
       * parent directories preserved) to the `destDir`.
       *
       * Default: `[]`
       */
      ...
      /**
       * One or more matcher expression (regular expression, glob string, or function). Files within
       * the node whose names match this expression will not be copied to the `destDir` if they
       * otherwise would have been.
       *
       * _Note_: in the case when a file matches both an include and exclude pattern, the exclude
       * pattern wins
       *
       * Default: `[]`
       */
       ...
       * `relativePath` as its first argument. The value returned from `getDestinationPath` will
       * be used as the destination for the new node. This is a very simple way to rename files
       * or move them from one path to another (replacing the need for `broccoli-file-mover`
       * for example).
       *
